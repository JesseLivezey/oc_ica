...
class SC_Soft(SC_Optimizer):
    ...
    def __init__(self, lambd, **fit_kwargs):
        self.L = theano.shared(np.array(1.).astype('float32'))
        super(SC_Soft, self).__init__(lambd, **fit_kwargs)
    ...
    def transforms(self, W, X, n_steps=25):
        ...
        def step(x_km1, y_k, t, W, X, lambd, L):
            cost = self.reconstruction_cost(W, X, y_k) + lambd * self.prior_cost(y_k)
            rec_grad = T.grad(cost, y_k)
            x_k = y_k - (X.shape[1].astype('float32')*rec_grad/L)
